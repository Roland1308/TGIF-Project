<template>
  <div>
    <div class="row">
      ...
      <div class="col-6 principale">
        <h2 class="centered">Senate at a glance</h2>
        <table class="table table-striped table-bordered table-hover">
          ...
          <tbody id="Table_at_Glance">
            <tr>
              <td>Democrats</td>
              <td>{{statisticsSenate.NrOfDemo}}</td>
              <td>{{statisticsSenate.AvgOfDemo + "%"}}</td>
            </tr>
            <tr>
              <td>Republicans</td>
              <td>{{statisticsSenate.NrOfRepu}}</td>
              <td>{{statisticsSenate.AvgOfRepu + "%"}}</td>
            </tr>
            <tr>
              <td>Independents</td>
              <td>{{statisticsSenate.NrOfInde}}</td>
              <td>{{statisticsSenate.AvgOfInde + "%"}}</td>
            </tr>
          </tbody>
          <tfoot id="Foot_at_Glance">
            <tr>
              <td>Total</td>
              <td>{{statisticsSenate.TotalReps}}</td>
              <td>{{statisticsSenate.TotalPerc + "%"}}</td>
            </tr>
          </tfoot>
        </table>
      </div>
      ...
      <div class="col-6">
        ...
        <table class="table table-striped table-bordered table-hover table-sm">
          ...
          <tbody id="Table_Least_Eng">
            <tr v-for="(person, idx) in mostMembersSenateA" :key="idx">
              <td>
      ...
      <div class="col-6">
        ...
        <table class="table table-striped table-bordered table-hover table-sm">
          ...
          <tbody id="Table_Most_Eng">
            <tr v-for="(person, idx) in leastMembersSenateA" :key="idx">
              <td>
                <a v-bind:href="person.url" target="_blank">
                  {{person.first_name}} {{person.middle_name}}
                  {{person.last_name}}
...
export default {
  ...
  computed: {
    ...
    leastMembersSenateA() {
      return this.$store.state.leastMembersSenateA;
    },
    mostMembersSenateA() {
      return this.$store.state.mostMembersSenateA;
    }
  },
